Понял. Вот точный текст для вашего README.md с сохранением оригинального оформления, вашими прямыми ссылками на установку и вырезанным блоком донатов:
...
Установить/удалить патч для обхода блокировки протокола OpenVPN (только для UDP соединений)

Bash
/root/antizapret/patch-openvpn.sh [1-4]
Включить/отключить OpenVPN DCO
...
Как переустановить сервер и сохранить настройки и работоспособность ранее созданных файлов подключений OpenVPN (.ovpn) и WireGuard/AmneziaWG (.conf)?
Выполните команду
...
Как пересоздать все файлы подключений (*.ovpn и *.conf) в подпапках /root/antizapret/client?
Выполните команду
...
Bash
/root/antizapret/parse.sh ip
Как работает опциональная защита SSH?
