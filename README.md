...
**12. Почему Telegram иногда зависал намертво и требовал перезапуска?**
...
**14. Есть ли готовый инструмент для проверки, что именно заблокировано у клиента?**
...
**15. Как решается проблема с российской геолокацией у Cloudflare WARP?**
При авторегистрации Cloudflare сам решает, на какой узел (colo) отправить ваш туннель, и нередко это московский узел `DME` — с апреля 2026 трафик через него режется DPI, часть сайтов не открывается даже при рабочем WARP. Сервер использует [warpscout](https://github.com/vernette/warpscout) — он перебирает публичные адреса эндпоинтов Cloudflare WARP и находит рабочий, который не выходит через `DME`. Публичный ключ пира WARP один на все эндпоинты, поэтому найденный адрес можно подставить в уже зарегистрированный на сервере ключ, не трогая саму регистрацию.
Механизм автоматический и не требует ничего от вас:
* Ставится только если провайдером WARP-style egress выбран Cloudflare (для Proton не нужен — там точка подключения указана явно вами).
* Полное сканирование занимает несколько минут, поэтому оно **не выполняется при каждой перезагрузке** — результат сохраняется в `/etc/wireguard/warpscout-endpoint` и обновляется не чаще раза в 6 дней в фоне, во время ночного `update.sh`. `up.sh` при поднятии туннеля только читает уже готовый файл.
* Если `warpscout` не установился или сканирование ничего не нашло, используется обычный эндпоинт, который предложила сама Cloudflare при регистрации — деградация полностью безопасна.
